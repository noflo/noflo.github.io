...
~~~
(did you know / helpful tricks box?) if you do not specify an `in` port it will use the inPort named `in` by default if it exists, the same for `out`
~~~
...
~~~
(did you know / helpful tricks box?) When you send an IP, if you do not send an openBracket and closeBracket explicitly, or let the forwardBrackets do the wrapping for you, every packet you send will emit its own openBracket and closeBracket.
~~~
