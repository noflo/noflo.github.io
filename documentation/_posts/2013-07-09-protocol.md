...
Get the JSON description of a graph.

* `graph`: graph the action targets

### `list`

List the graphs on the runtime. The runtime will respond by a series
of `graph` message followed by a `graphsdone` message.

### `graph`

Send the description of a graph on the runtime, in response to a `get`
or `list` message. The runtime should send one `graph` message per
graph available on the runtime.

* `graph`: the graph identifier
* `description`: the JSON description of the graph

### `graphsdone`

Signal the end of the listing of the graphs from the runtime.
...
List the networks on the runtime. The runtime will respond by a serie
of `network` messages followed by a `networksdone` message.

### `network`

Send the description of a network on the runtime, in response to a
`list` message. The runtime should send one `network` message per
network available on the runtime.

* `graph`: graph the identifier this network is running
* `started`: whether the network is started

### `networksdone`

Signal the end of the listing of the networks from the runtime.

## Protocol implementations

* [noflo-ui](https://github.com/noflo/noflo-ui) is a client for the FBP protocol
* [noflo-runtime-base](https://github.com/noflo/noflo-runtime-base) is a transport-independent implementation of the protocol for NoFlo, client-side and server-side JavaScript programming
