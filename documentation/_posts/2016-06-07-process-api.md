...
If you need to do something advanced and the [Get](#Get) and [Stram](#Stream) helpers cannot do what you need, you can read information right from the buffer. To do that easily, there are `input.buffer` helpers.
...
instead of reaching into the component ports using `component.outPorts.portname`
With the introduction of the process api, we now can use `output.ports.portname`
The same is true with *In Ports*, `input.ports.portname`
@TODO: ^ change to say modern way now and touch on old way in Legacy
...
Bracket forwarding is a way to pass on brackets so that you
